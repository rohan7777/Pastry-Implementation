...
let numberOfDigits =
    Math.Ceiling(Math.Log(numberOfNodes |> float) / Math.Log(16.0))
    |> int
...
for i in [ 1 .. numberOfNodes - 1 ] do
    ...
    nodeId <-
        (String.replicate ((numberOfDigits |> int) - hexLen) "0")
        + hexID
    ...
    let zeroth =
        String.replicate (numberOfDigits |> int) "0"
...
printfn "Killing %d nodes" numberOfFailureNodes

let actorsArray =
    nodeMap |> Map.toSeq |> Seq.map fst |> Seq.toArray

let mutable counter = 0

let mutable deadNode =
    String.replicate (numberOfDigits |> int) "0"

while counter < numberOfFailureNodes do
    while deadNode = String.replicate (numberOfDigits |> int) "0"
          || deadNodeSet.Contains deadNode do
...
while (k <= numberOfRequests) do
    for source in actorsArray do
        if not (deadNodeSet.Contains source) then
            ...
            while (destinationId = source
                   || (deadNodeSet.Contains destinationId)) do
